/* Kernel2.C kernel project        */
/* April 25, 2008 - CS421 - TS      */
/* By Jeremy VillEgAS              */
...
struct queue_t *q=NULL,*SL=NULL,*UL=NULL,*WL=NULL,*RDL=NULL;
...
struct queue_t* delete(struct queue_t *q,struct pcb_t *p){
  struct queue_t *que_cur=q,*que_prev=NULL;
  /* no items in the queue */
...
struct queue_t* set_pcb_status(struct queue_t* s,enum stateType t){
  struct queue_t *q=UL;
  s->pcb_item->pcb_status=t;
  if(s->pcb_item->pid>SYSTEM_QUEUE_THRESHOLD)
    q=SL;
  while(q->pcb_item!=NULL){
    if(q->pcb_item->pid==s->pcb_item->pid)
      q->pcb_item->pcb_status=t;
    q=q->que_next;
  }
  return(q);
}
...
int invoke_scheduler(){
  struct pcb_t *c=NULL;
  /* check the ready list see whats next */
  if(RDL==NULL)return(0);
  /* see whats curerntly running */
  c=RDL->pcb_item;   
  /* if whats currently running has a higher prio than what is next */
  /* do nothing */
  /* otherwise set the current process to waiting */
  c->pcb_status=running;
  /* run the new process */ 
  return(0);
}
 
struct queue_t *suspend_operation(struct queue_t* q,struct pcb_t p){}
struct queue_t *activate_operaetion(struct queue_t* q,struct pcb_t p){}
...
int queue_t* release_opeartion(struct pcb_t *p,int){
  struct rcb_t *t=NULL;
